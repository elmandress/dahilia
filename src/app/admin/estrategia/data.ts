...
export const NEXT_ACTIONS: TodoAction[] = [
  ...
  { id: 'resenas-en-la-home', label: '(Mati) Prender la sección de reseñas de Google en la home', detail: 'La sección ya está hecha: muestra la puntuación, cuántas opiniones hay y las 3 últimas reseñas reales, con el nombre y la foto de cada clienta y un link para abrirlas en Google. Le falta la llave: en Google Cloud (el mismo proyecto de Search Console), activá "Places API (New)", creá una clave de API con facturación activada y ponela en Netlify como GOOGLE_PLACES_API_KEY. Sin eso, la sección no aparece (no muestra nada inventado). Las reseñas se piden en el momento, no se guardan: Google no permite guardarlas. Con el tráfico de hoy entra en el tramo gratis de 1.000 llamadas por mes.', horizon: 'ya' },
  { id: 'places-tope', label: '(Mati) Ponerle tope a la API de Places para que no pueda cobrar', detail: 'La SKU que trae las reseñas ("Place Details Enterprise + Atmosphere") tiene 1.000 llamadas gratis por mes, y lo que pasa de ahí se cobra solo (USD 25 cada 1.000). Con el tráfico de hoy no se llega ni cerca, pero para dormir tranquilo: Google Cloud → APIs y servicios → Places API (New) → Cuotas → poné un tope diario de 100 solicitudes. Si algún día se pasa, deja de responder y la sección de reseñas no se muestra. Nunca una factura sorpresa.', horizon: 'ya' },
  ...
  { id: 'pedir-indexacion', label: '(Mati) Pedir a mano la indexación de las 2 fichas que faltan', detail: 'Al 15/09 las notas del blog y /tienda/sweaters ya entraron. Quedan /tienda/sweater-cherry y /tienda/top-race: Search Console → Inspección de URLs → pegá cada una → "Probar URL publicada" → "Solicitar indexación". A Sweater cherry Google la vio con error 404 el 14/09, por una falla que se arregla con el próximo deploy (la copia de respaldo del catálogo no la tenía); hoy da bien. No uses npm run index-urls: Google acepta el aviso pero no lo registra (esa API es solo para avisos de empleo y transmisiones en vivo).', horizon: 'ya' },
