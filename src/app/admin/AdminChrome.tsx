...
// (encargos, productos, postulaciones), lo de configuración una vez, abajo.
const NAV_ITEMS = [
  {
    href: '/admin',
    label: 'Inicio',
...
export default function AdminChrome({ children }: { children: React.ReactNode }) {
  ...
  // Pendientes por sección (encargos nuevos, postulaciones nuevas) para los
  // badges del menú. Se carga una vez por sesión; el layout persiste entre rutas.
  ...
  useEffect(() => {
    ...
    const loadPending = async () => {
      const [ordersRes, weaversRes] = await Promise.all([
        supabase.from('custom_orders').select('id', { count: 'exact', head: true }).eq('status', 'new'),
        // La tabla de tejedoras puede no existir todavía — el error se ignora.
        supabase.from('weaver_applications').select('id', { count: 'exact', head: true }).eq('status', 'new'),
      ])
      setPending({
        '/admin/encargos': ordersRes.count ?? 0,
        '/admin/tejedoras': weaversRes.error ? 0 : (weaversRes.count ?? 0),
      })
    }
    loadPending()
  }, [isLoginPage])
  ...
  return (
    <div className="admin-layout">
      ...
      {/* Mobile logout — visible on small screens without opening the drawer */}
      <button
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="admin-sidebar-footer">
          <div className="admin-user-info">
            ...
            <span className="admin-user-email">{userEmail || 'Cargando...'}</span>
          </div>
